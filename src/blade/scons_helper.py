...
def generate_resource_index(target, source, env):
    ...
    print('#ifndef %s\n#define %s' % (guard_name, guard_name), file=h)
    print('''
...
''', file=h)
    ...
    print('// This file was automatically generated by blade\n', file=c)
    print('#include "%s"\n' % res_header_path, file=c)
    print('const struct BladeResourceEntry %s[] = {' % res_index_name, file=c)
    for s in source:
        src = str(s)
        var_name = blade_util.regular_variable_name(src)
        org_src = blade_util.relative_path(src, source_path)
        print('// %s' % org_src, file=h)
        print('extern const char RESOURCE_%s[%d];' % (var_name, s.get_size()), file=h)
        print('extern const unsigned RESOURCE_%s_len;\n' % var_name, file=h)
        print('    { "%s", RESOURCE_%s, %s },' % (org_src, var_name, s.get_size()), file=c)
    print('};', file=c)
    print('const unsigned %s_len = %s;' % (res_index_name, len(source)), file=c)
    print('// Resource index', file=h)
    print('extern const struct BladeResourceEntry %s[];' % res_index_name, file=h)
    print('extern const unsigned %s_len;' % res_index_name, file=h)
    print('\n#ifdef __cplusplus\n} // extern "C"\n#endif\n', file=h)
    print('\n#endif // %s' % guard_name, file=h)
    c.close()
    h.close()
    return None
...
def generate_shell_test_data(target, source, env):
    ...
    for i in range(0, len(source), 2):
        print(os.path.abspath(str(source[i])), source[i + 1], file=testdata)
    testdata.close()
    return None
...
def generate_shell_test(target, source, env):
    ...
    print('# Auto generated wrapper shell script by blade\n', file=script)
    print('set -e\n', file=script)
    for s in source:
        print('. %s' % os.path.abspath(str(s)), file=script)
    print(file=script)
    script.close()
    os.chmod(target, 0o755)
    return None
...
def generate_version_file(top_env, blade_root_dir, build_dir,
    ...
    version_cpp.write('extern const int kSvnInfoCount = %d;\n' % svn_info_len)
    version_cpp.write('extern const char* const kSvnInfo[%d] = {%s};\n' % (
            svn_info_len, ', '.join(['"%s"' % str(v) for v in svn_info_map.values()])))
    version_cpp.write('extern const char kBuildType[] = "%s";\n' % profile)
    version_cpp.write('extern const char kBuildTime[] = "%s";\n' % time.asctime())
    version_cpp.write('extern const char kBuilderName[] = "%s";\n' % os.getenv('USER'))
    version_cpp.write(
            'extern const char kHostName[] = "%s";\n' % socket.gethostname())
    compiler = 'GCC %s' % gcc_version
    version_cpp.write('extern const char kCompiler[] = "%s";\n' % compiler)
    version_cpp.write('}}\n')
